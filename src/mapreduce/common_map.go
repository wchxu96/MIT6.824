...
func doMap(
	...
	if err != nil {
		panic("cannot read the input file.")
	}
	kvSlice := mapF(inFile, string(fileContents))
	for _, kvPair := range kvSlice {
		k := kvPair.Key
		bin := ihash(k) % nReduce
		res[bin] = append(res[bin], kvPair)
	}
	for r, kvSliceHash := range res {
